...
# test model13 for Italy because incidence_case is not known for the whole period
M_model13 = stan_model("models/hauser/model13d.stan")
data_list_model13$p_psi=c(71,18)  # We need to set this for there to be no exceptions so use case A below for now
...
print(T_model13,pars=c("beta","epsilon","rho","pi"))


# Create data and bash files ----
# symptomatic = 80% (A) (Bi et al) 
#data_list_model13$p_psi=c(71,18) 
#bashfile_rdump("model13d",id="SE-A",data_list_model13,warmup=500,iter=500,adapt_delta=0.8,max_depth=10,init=0.5,timelimit=2,chains=4)
...
#bashfile_rdump("model13d",id="SE-B",data_list_model13,warmup=500,iter=500,adapt_delta=0.8,max_depth=10,init=0.5,timelimit=2,chains=4)
...
print(S_model13ITB,pars=c("beta","epsilon","rho","pi","psi"),digits_summary=3)
